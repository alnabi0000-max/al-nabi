...
export function TemplateGridCard({ template, onOpen }: Props) {
  ...
  return (
    <button
      ...
      className={clsx(
        "group relative overflow-hidden rounded-2xl border border-white/[0.08] bg-zinc-950/80 text-left",
    ...
    >
      <div className="relative aspect-video overflow-hidden bg-zinc-900">
        ...
        <span className="pointer-events-none absolute left-2.5 top-2.5 z-[3] rounded-md bg-black/70 px-2 py-0.5 text-[10px] font-medium uppercase tracking-wider text-zinc-200">
          {template.category}
        </span>
        <span className="pointer-events-none absolute inset-0 z-[3] flex items-center justify-center opacity-0 transition group-hover:opacity-100">
          <span className="flex h-10 w-10 items-center justify-center rounded-full border border-white/20 bg-black/55">
